...
define(['bower_components/om-echarts/init', 'angularAMD'], function (echarts, angularAMD) {

    angularAMD.directive('ngEcharts',
        function () {
        var charts = [];
        var connectList = [];
        return {
            ...
            link: function(scope,element,attrs,ctrl){
                ...
                if (attrs.id && scope.config.connect) {
                ...
                function downTheme(name){
                    ...
                    require(['bower_components/om-echarts/theme/' + themeName], function (th) {
                        theme = th;
                        downThemeOk = true;
                        refreshChart();
                    });
                ...
                function connect() {
                    if (connectList && connectList.length > 0) {
                        for (var i in connectList) {
                            if (attrs && attrs.id && attrs.id == connectList[i]['key']) {
                                var others = [];
                ...
                function refreshChart(){
                    if (!downThemeOk && scope.config.theme) {
                        downTheme(scope.config.theme);
                    }
